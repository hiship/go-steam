...
func beginAuthSession(crypt string, accountName string, timestamp *uint64) (*pb.CAuthentication_BeginAuthSessionViaCredentials_Response, error) {
	...
	err := writer.WriteField("input_protobuf_encoded", base64.StdEncoding.EncodeToString(data))
	if err != nil {
		return nil, err
	}
	err = writer.Close()
...
func (session *Session) finalizeLogin(pollAuth *pb.CAuthentication_PollAuthSessionStatus_Response) error {
	...
	err := writer.WriteField("nonce", *pollAuth.RefreshToken)
	if err != nil {
		return err
	}
	err = writer.WriteField("sessionid", session.sessionID)
	if err != nil {
		return err
	}
	err = writer.WriteField("redir", "https://steamcommunity.com/login/home/?goto=")
	if err != nil {
		return err
	}
	err = writer.Close()
	...
	for _, info := range loginFinalized.TransferInfo {
		...
		err := writer.WriteField("nonce", info.Params.Nonce)
		if err != nil {
			return err
		}
		err = writer.WriteField("auth", info.Params.Auth)
		if err != nil {
			return err
		}
		err = writer.WriteField("steamID", loginFinalized.SteamID.ToString())
		if err != nil {
			return err
		}
		err = writer.Close()
...
func (session *Session) Login(accountName, password, sharedSecret string, timeOffset time.Duration) error {
	...
	session.expireTime = time.Now().Add(2 * 24 * time.Hour)
...
func (session *Session) IsLogged() bool {
	// 如果 sessionID 为空，表示没有登录
	if session.sessionID == "" || session.oauth.SteamID == 0 {
		return false
	}
...
func (session *Session) Refresh() error {
	...
	if err != nil {
		session.sessionID = ""
		session.oauth = OAuth{}
		session.client.Jar = nil // 清除 cookies
		session.expireTime = time.Now().Add(-5 * time.Minute)
		return err
	}
	...
	for _, cookie := range resp.Cookies() {
		println(cookie.Name)
		if cookie.Name == "steamRefresh_steam" {
			session.client.Jar.SetCookies(&url.URL{Scheme: "https", Host: "login.steampowered.com"}, []*http.Cookie{cookie})
			break
		}
	}
	return nil
