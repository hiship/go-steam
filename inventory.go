...
func (session *Session) fetchInventory(
	...
	if resp == nil {
		return false, 0, errors.New("invalid response")
	}
	type Asset struct {
		AppID      uint32 `json:"appid"`
		ContextID  uint64 `json:"contextid,string"`
	...
	type Response struct {
		...
		HasMore             int             `json:"more_items,omitempty"`
	...
	for i, desc := range response.Descriptions {
		key := fmt.Sprintf("%s_%s", desc.ClassID, desc.InstanceID)
		descriptions[key] = i
	}
...
func (session *Session) GetInventory(sid SteamID, appID, contextID uint64, tradableOnly bool) ([]InventoryItem, error) {
	...
	if tradableOnly {
		filters = append(filters, IsTradable(1))
	}
